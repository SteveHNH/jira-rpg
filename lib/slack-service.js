...
export async function validateCommandContext(channelId, userId, guildName = null) {
  try {
    ...
    if (guildName) {
      const { getGuildByName } = await import('./guild-service.js');
    ...
    const { getGuildsByUser } = await import('./guild-service.js');
    const { getUserBySlackId } = await import('./user-service.js');
