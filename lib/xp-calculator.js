...
export function calculateXpAward(payload) {
  ...
  if (!issue || !changelog) {
    return { xp: 0, reason: 'Invalid payload' };
  }
  ...
  if (!statusChange) {
    return { xp: 0, reason: 'No status change detected' };
  }
  ...
  } else if (fields.assignee && fromStatus === 'To Do') {
...
export function checkLevelUp(oldXp, newXp) {
  ...
  return {
    ...
    xpToNextLevel: leveledUp ? getXpForNextLevel(newLevel) - newXp : getXpForNextLevel(oldLevel) - newXp
  };
}
