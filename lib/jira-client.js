...
// Removed axios dependency - using native fetch for better serverless performance
...
export class JiraClient {
  constructor() {
    ...
    // Create native fetch-based HTTP client for better serverless performance
    this.defaultHeaders = {
  ...
  /**
   * Native fetch wrapper with timeout and error handling
  ...
  async executeJqlQuery(jql, maxResults = 50) {
    try {
      ...
      const response = await this.fetchWithTimeout('/rest/api/2/search', { params });
  ...
  async healthCheck() {
    try {
      ...
      const response = await this.fetchWithTimeout('/rest/api/2/serverInfo', { 
