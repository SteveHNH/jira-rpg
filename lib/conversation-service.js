  ...
  try {
    ...
    if (hasNarratives) {
      ...
      const simplifiedTickets = stories.map(story => ({
        ...
        timeSpent: story.ticketData?.timeSpent || null,
        ticketType: story.ticketData?.ticketType || 'task',
        project: { key: story.project?.key || story.ticketData?.project || 'PROJ' },
        components: story.components || story.ticketData?.components || [],
        // Include saved XP data for richer context
        xpAwarded: story.xpAwarded || 0,
        source: 'saved_story'
      }));
    } else {
    ...
    // Build prompt for conversational model (always call Ollama for fresh epic tales)
    ...
    return {
      ...
      metadata: {
        ...
        conversationalGeneration: true, // Always true now
        dataSource: hasNarratives ? 'saved_stories' : 'jira_api'
      }
    };
