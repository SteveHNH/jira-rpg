...
export async function getRecentStories(userJiraUsername, limitCount = 10) {
  try {
    ...
    // TEMPORARY: Query without orderBy to avoid composite index requirement
    // TODO: Create composite index for userJiraUsername + createdAt in Firebase console
    const q = query(
      storiesRef,
      where('userJiraUsername', '==', userJiraUsername),
      limit(limitCount * 2) // Get more to compensate for client-side sorting
    );
    ...
    // Sort on client side (temporary workaround)
    stories.sort((a, b) => {
    ...
    console.log(`Retrieved ${limitedStories.length} recent stories for user: ${userJiraUsername}`);
