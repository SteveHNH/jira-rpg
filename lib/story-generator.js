...
PRIORITY: ${priority}${storyPoints ? `\nSTORY_POINTS: ${storyPoints}` : ''}`;
  ...
  // Generous timeout for Ollama server processing (especially longer stories)
  const timeoutMs = 30000; // 30s timeout to accommodate slow Ollama responses
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      console.warn(`Story generation timeout after ${timeoutMs}ms, using fallback`);
    } else {
      console.error('Story generation failed:', error);
    }
    
    // Return fallback story if AI fails or times out
...
export async function routeStoryToGuilds(webhookPayload) {
  try {
    ...
    const storyDataForDm = {
      narrative: storyData.story, // Extract the actual story text
