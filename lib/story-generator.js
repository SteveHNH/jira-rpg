 ...
 * @param {Object} userData - User data with guild memberships
 * @returns {Promise<Object>} - Story routing results
 */
export async function routeStoryToGuilds(webhookPayload, userData = null) {
  try {
    ...
    // Find matching guilds based on ticket components/labels
    const allMatchingGuilds = await findGuildsForTicket(components, labels);
    console.log('Found guilds matching ticket components/labels:', allMatchingGuilds.length, allMatchingGuilds.map(g => g.name));
    
    // Filter guilds by user membership (only send to guilds where user is a member)
    let matchingGuilds = allMatchingGuilds;
    if (userData && userData.guilds) {
      const userGuildNames = userData.guilds;
      matchingGuilds = allMatchingGuilds.filter(guild => 
        userGuildNames.includes(guild.name)
      );
      console.log('Filtered guilds by user membership:', matchingGuilds.length, matchingGuilds.map(g => g.name));
      console.log('User guild memberships:', userGuildNames);
    }
    
    if (matchingGuilds.length === 0) {
      const reason = userData && userData.guilds 
        ? 'No guilds found where user is a member that also match ticket components/labels'
        : 'No guilds matched ticket components/labels';
      console.log(reason);
      return {
        success: true,
        routedChannels: 0,
