...
// Helper function to get user by Slack ID with fallback strategy
export async function getUserBySlackId(slackUserId) {
  try {
    console.log('Starting Firebase query for slackUserId:', slackUserId);
    
    // Strategy 1: Try the where() query (this might be hanging)
    try {
      console.log('Attempting where() query...');
      const usersRef = collection(db, 'users');
      const q = query(usersRef, where('slackUserId', '==', slackUserId));
      
      console.log('Executing Firebase query...');
      
      // Add internal timeout to the Firebase query itself
      const queryTimeout = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Firebase where() query timeout')), 8000)
      );
      ...
      if (!querySnapshot.empty) {
        ...
        console.log('User found via where() query:', userData.id, 'jiraUsername:', userData.jiraUsername);
        return userData;
      }
      
    } catch (queryError) {
      console.error('where() query failed:', queryError.message);
      
      // Strategy 2: Fallback to scanning all users (slower but more reliable)
      console.log('Falling back to full collection scan...');
      try {
        const usersRef = collection(db, 'users');
        const allUsersSnapshot = await getDocs(usersRef);
        
        console.log(`Scanning ${allUsersSnapshot.size} users for slackUserId match...`);
        
        for (const userDoc of allUsersSnapshot.docs) {
          const userData = userDoc.data();
          if (userData.slackUserId === slackUserId) {
            console.log('User found via scan:', userDoc.id, 'jiraUsername:', userData.jiraUsername);
            return {
              id: userDoc.id,
              ...userData
            };
        ...
        console.error('Collection scan also failed:', scanError.message);
        throw queryError; // Throw the original query error
      }
    }
