...
export async function getUserBySlackId(slackUserId) {
  console.log('getUserBySlackId called with:', slackUserId);
  ...
  if (querySnapshot.empty) {
    // Debug: Let's see what users exist (for debugging only)
    const allUsersSnapshot = await getDocs(usersRef);
    console.log('All users in database:');
    allUsersSnapshot.docs.forEach(doc => {
      const data = doc.data();
      console.log(`  - Doc ID: ${doc.id}, slackUserId: ${data.slackUserId}, jiraUsername: ${data.jiraUsername}`);
    });
    return null;
  }
  ...
  console.log('Found user:', userData.id, 'with slackUserId:', userData.slackUserId);
  return userData;
}
